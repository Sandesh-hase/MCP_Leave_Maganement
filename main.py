...
def apply_leave(employee_id: str, leave_dates: List[str]) -> str:
    ...
    if available_balance < requested_days:
        return (f"Insufficient leave balance. You requested {requested_days} "
                f"day(s) but have only {available_balance}.")
...
# Tool: Cancel Leave for specific dates
@mcp.tool()
def cancel_leave(employee_id: str, leave_dates: List[str]) -> str:
    """
    Cancel leave for specific dates (e.g., ["2025-04-17", "2025-05-01"])
    """
    if employee_id not in employee_leaves:
        return "Employee ID not found."
    ...
    for date in leave_dates:
        if date in history:
            ...
            return f"Leave date {date} not found in history."

    return (f"Leave for dates {', '.join(leave_dates)} has been cancelled. "
            f"Updated balance: {employee_leaves[employee_id]['balance']}.")
